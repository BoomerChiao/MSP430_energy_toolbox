...
const int SENSOR_A = 18;   //  P2.2 → Energia pin 18
const int SENSOR_B = 17;   //  P3.2 → Energia pin 17
const int BUTTON_A = 8;   //  P2.7 → Energia pin 8
const int BUTTON_B = 21;   //  P2.3 → Energia pin 21
...
const int idleDelay = 1200; // 靜止多少毫秒才刷新
...
void loop() {
  ...
  if (state != lastState && millis() - lastActiveTime < sleepTimeout) {
    ...
    if (!abTriggered) { //LOAD...
     OLED_Draw18x23(imageL, 21, 1);
     OLED_Draw18x23(imageO, 40, 1);
     OLED_Draw18x23(imageA, 61, 1);
     OLED_Draw18x23(imageD, 80, 1);
    abTriggered = true;   // 鎖住，只印一次
  }
