...
/**
  ...
  */
  unsigned char image[1050]; //RAM 不夠,完整 buffer 要  = 4000 bytes
  int buffer_height;
...
void loop()
{
  ...
  for (int i = 0; i < 4; i++){
    ...
    paint.Clear(UNCOLORED);// 清字幕區（整個 buffer）
    paint.DrawStringAt(8, 16, "HEL122LO", &Font16, COLORED);
    epd.Display1(image);
  }
  delay(2000);
  ...
  for (int i = 0; i < 4; i++){
    Paint paint(image, epd.bufwidth*8, epd.bufheight);
    paint.Clear(UNCOLORED);// 清字幕區（整個 buffer）
    paint.DrawStringAt(8, 16, "asas", &Font16, COLORED);
  ...
  paint.Clear(UNCOLORED);
  paint.DrawStringAt(8, 16, "HEL122LO", &Font12, COLORED);
  epd.Display1(image);//2
  
  paint.Clear(UNCOLORED);
  paint.SetRotate(ROTATE_90);// 設定橫向
  paint.DrawStringAt(8, 16, "HELLO", &Font12, COLORED);
  epd.Display1(image);//3
  ...
  /*
  epd.Init(PART);//進入局部刷新模式
  Paint paint(image, epd.bufwidth*8, epd.bufheight);
  paint.Clear(UNCOLORED);  // 清字幕區（整個 buffer）
  paint.DrawStringAt(8, 8, "12444kk3", &Font16, COLORED);
  epd.DisplayPart(image);//局部刷新
  delay(1000);*/
